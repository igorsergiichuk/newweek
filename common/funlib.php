...
function imageFinder($picture)
{
    if ($picture != '')
    {
        $imageLink = '<p class="image"><img src="http://localhost/news/pictures/' . $picture . '" width="600" height="400"></p>';
        return $imageLink;
    }
}
...
function showTitle($arr)
{
    $html = '';
    foreach ($arr as $key => $value) {
        $html .=  ($key + 1) . '. <a href="posts/'. $value['url'] . '" class="articlesurl">' .  ucfirst($value['title']) . '</a> ';
    }
    return $html;
}
...
function separatePost($arr)
{
    $html = '';
    foreach ($arr as $key => $value) {
        $html .= '<form action="http://localhost/news/posts/' . $value['url']. '" method="get" class="searchRes">';
...
function sepPagePostsLimited($arr)
{
    ...
    foreach ($arr as $key=>$value)
    {
        ...
        echo '<p class="articles">' . $post_date . '</p>';
        echo imageFinder($post_image);
        echo '<p class="postText">' . $post_text . '...</p>' . '<p class="postText"><b>To view full text <a href="http://localhost/news/register.php">register</a> or <a href="http://localhost/news/login.php">login</a></b></p>';
        echo tags($tags);
    }
}
...
{
    $sql = $db->query("SELECT  `comments`.`text_com` ,  `comments`.`date_com` ,  `comments`.`user_id` ,  `comments`.`id` ,  `users`.`username` ,  `users`.`email`, SUM(  `likes`.`likes` ) AS  `sumL` , SUM(  `likes`.`dislikes` ) AS  `sumD`, (SUM(  `likes`.`likes` )- SUM(  `likes`.`dislikes` )) AS `all`
                       FROM  `comments` 
