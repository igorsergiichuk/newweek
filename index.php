...
foreach ($categories as $key=>$value)
{
    ...
    $html .= '<a href="categories/' . $category . '.' . "php" . '"><h2>' . ucfirst($category) . '</h2></a><br>';
    $posts = $db->query("SELECT `title`, `url` FROM `news` WHERE `category`= '{$category_id}' LIMIT 5");
...
foreach ($pictures as $number => $arr) {
    ...
    $imgPath = "http://localhost/news/pictures/" . $pictureName;
